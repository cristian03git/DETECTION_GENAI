...
def train_amp_with_valcurve(model, train_loader, val_loader, epochs, start_epoch=0, history=None):
    ...
    for ep in range(start_epoch, epochs):
        ...
        # Early stopping
        if val_loss < best_val_loss - global_var["min_delta"]:
            best_val_loss = val_loss
            best_model_state = copy.deepcopy(model.state_dict())
            patience_counter = 0
        else:
            patience_counter += 1
            if patience_counter >= global_var["patience"]:
                print(f"[LOG] Early stopping triggered after {ep+1} epochs.")
                break

    if best_model_state is not None:
      model.load_state_dict(best_model_state)
      print("[LOG] Loaded best model state based on validation loss.")
